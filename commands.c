...
void handleUndo()
{
    for (int k = 0; (k < i1) && (actual_command->prev != NULL); ++k)
    {
        if (actual_command->c == 'd')
            actual_mod = actual_mod->next;
        actual_command = actual_command->prev;
    }
}

void handleRedo()
{
    for (int k = 0; (k < i1) && (actual_command->next != NULL); ++k)
    {
        if (actual_command->c == 'd')
            actual_mod = actual_mod->prev;
        actual_command = actual_command->next;
    }
}
